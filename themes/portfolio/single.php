      ...
      while(have_posts()) {
        ...
        <?php 
          ...
          if (get_post_field('live_link', get_the_ID()) AND get_post_field('repo_link', get_the_ID())) {
            ...
            echo '<a class="single__container__live button single" href="'.esc_url(get_post_field('live_link', get_the_ID())).'" target="_blank">See Live.</a>';
            echo '<a class="single__container__git button" href="'.esc_url(get_post_field('repo_link', get_the_ID())).'" target="_blank">See Repo.</a>';            
            echo '</div>';
          ...
          the_content(); ?>
        <h4><a href="<?php echo esc_url(site_url('/blog')) ?>">&larr; Back to all projects</a></h4>
    <?php  
    echo "</div>";
    } 
    ?>
